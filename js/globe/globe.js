...
define(["require", "exports", "./Common", "./SunCalculations", "../third-party/Tween", "../third-party/faker.min"], function (require, exports, Common, SunUtils, TWEEN, faker) {
    "use strict";
    var debugMode = false;
    var MAX_EVENTS = 30;
    var Globe;
    (function (Globe_1) {
        var sunCalc = new SunUtils.Calculator(new Date());
        var PI_HALF = Math.PI / 2;
        var DIST_MAX = 1500;
        var DIST_MIN = 350;
        var EARTH_RADIUS = 200;
        var LINE_COLOR = 0x3498db;
        var SUN_DECLINATION = sunCalc.getDeclanation();
        var SUN_RIGHT_ASCENSION = sunCalc.getRightAscension();
        var UNIVERSE_RADIUS = 1600;
        var Globe = (function () {
            function Globe(container, manager) {
                var _this = this;
                ...
                this.target = { x: Math.PI, y: Math.PI / 5.0 };
                this.targetOnDown = { x: 0, y: 0 };
                this.raycaster = new THREE.Raycaster();
                this.onMouseDown = function (event) {
                    event.preventDefault();
                    _this.container.addEventListener('mousemove', _this.onMouseMove, false);
                    _this.container.addEventListener('mouseup', _this.onMouseUp, false);
                    _this.container.addEventListener('mouseout', _this.onMouseOut, false);
                    _this.mouseOnDown.x = -event.clientX;
                    _this.mouseOnDown.y = event.clientY;
                    _this.targetOnDown.x = _this.target.x;
                    _this.targetOnDown.y = _this.target.y;
                    _this.container.style.cursor = 'move';
                    _this.shouldRotateGlobe = false;
                };
                this.onMouseWheel = function (event) {
                    event.preventDefault();
                    if (_this.overRenderer) {
                        _this.zoom(event.wheelDeltaY * 0.3);
                    }
                    return false;
                };
                this.onMouseOver = function (event) {
                    _this.overRenderer = true;
                };
                this.onMouseOutWindow = function (event) {
                    _this.overRenderer = false;
                };
                this.onMouseMoveWindow = function (event) {
                    // Track mouse movements for raytracing functionality
                    _this.mouse.x = (event.clientX / _this.container.offsetWidth) * 2 - 1;
                    _this.mouse.y = -(event.clientY / _this.container.offsetHeight) * 2 + 1;
                };
                this.onMouseMove = function (event) {
                    _this.mouse.x = -event.clientX;
                    _this.mouse.y = event.clientY;
                    var zoomDamp = _this.distance / 1000;
                    _this.target.x = _this.targetOnDown.x + (_this.mouse.x - _this.mouseOnDown.x) * 0.005 * zoomDamp;
                    _this.target.y = _this.targetOnDown.y + (_this.mouse.y - _this.mouseOnDown.y) * 0.005 * zoomDamp;
                    _this.target.y = _this.target.y > PI_HALF ? PI_HALF : _this.target.y;
                    _this.target.y = _this.target.y < -PI_HALF ? -PI_HALF : _this.target.y;
                };
                this.onMouseUp = function (event) {
                    _this.container.removeEventListener('mousemove', _this.onMouseMove, false);
                    _this.container.removeEventListener('mouseup', _this.onMouseUp, false);
                    _this.container.removeEventListener('mouseout', _this.onMouseOut, false);
                ...
                this.onMouseOut = function (event) {
                    ...
                    _this.container.removeEventListener('mouseup', _this.onMouseUp, false);
                    _this.container.removeEventListener('mouseout', _this.onMouseOut, false);
                    _this.shouldRotateGlobe = true;
                };
                this.onWindowResize = function (event) {
                    _this.camera.aspect = _this.container.offsetWidth / _this.container.offsetHeight;
                    _this.camera.updateProjectionMatrix();
                    _this.renderer.setSize(_this.container.offsetWidth, _this.container.offsetWidth);
                };
                this.zoom = function (delta) {
                    _this.distanceTarget -= delta;
                    _this.distanceTarget = _this.distanceTarget > DIST_MAX ? DIST_MAX : _this.distanceTarget;
                    _this.distanceTarget = _this.distanceTarget < DIST_MIN ? DIST_MIN : _this.distanceTarget;
                };
                this.fadeOutRemove = function (event, seconds) {
                            ...
                            (function (child) {
                                TweenLite.to(child.material, seconds / 1000, {
                                    ...
                                    onComplete: function () {
                                        --thing;
                                        if (thing == 0) {
                                            _this.scene.remove(child.parent);
                                        }
                                        if (child.tween) {
                                            setTimeout(function () {
                ...
                this.currentZoom = function () { return _this.distanceTarget; };
                this.setLocation = function (x, y, zoom) {
                    _this.distanceTarget = zoom;
                    _this.target.x = x;
                    _this.target.y = y;
                };
                this.toggleRotation = function () {
                    _this.shouldRotateGlobe = !_this.shouldRotateGlobe;
                };
                this.stopRotation = function () {
                    _this.shouldRotateGlobe = false;
                };
                this.isRotating = function () {
                    return _this.shouldRotateGlobe;
                };
                this.toggleSun = function () {
                    if (_this.ambientLight.color.getHex() == 0x404040) {
                        _this.ambientLight.color.setHex(0xFFFFFF);
                        _this.directionalLight.visible = false;
                    }
                    else {
                        _this.ambientLight.color.setHex(0x404040);
                        _this.directionalLight.visible = true;
                    }
                };
                this.toggleSunDebug = function () {
                    _this.sun.material.visible = !_this.sun.material.visible;
                    _this.sunPath.material.visible = !_this.sunPath.material.visible;
                    _this.equatorPath.material.visible = !_this.equatorPath.material.visible;
                };
                this.container = container;
                this.manager = manager;
                /*
                ...
                this.globeMesh = new THREE.Mesh(new THREE.SphereGeometry(EARTH_RADIUS, 40, 30, 0, Math.PI * 2, 0, Math.PI), new THREE.MeshPhongMaterial({
                ...
                this.sun = new THREE.Mesh(new THREE.SphereGeometry(3, 30, 30), new THREE.MeshBasicMaterial({
                    color: 0xff0000,
                    visible: debugMode
                }));
                ...
                this.sunPath = new THREE.Line(basePath.createPointsGeometry(50).rotateX(Math.PI / 2 + SUN_DECLINATION), new THREE.LineBasicMaterial({ color: 0x00ff00, visible: debugMode }));
                this.scene.add(this.sunPath);
                this.equatorPath = new THREE.Line(basePath.createPointsGeometry(50).rotateX(Math.PI / 2), new THREE.LineBasicMaterial({ color: 0x00ff00, visible: debugMode }));
                this.scene.add(this.equatorPath);
                var universeMesh = new THREE.Mesh(new THREE.SphereGeometry(UNIVERSE_RADIUS, 40, 30), new THREE.MeshBasicMaterial({
                ...
                TweenLite.to(sunInfo, 86400, {
                    ...
                    onUpdate: function () {
                        ...
                        _this.directionalLight.position.copy(vector);
                        _this.sun.position.copy(vector);
                    }
                });
                /*
                 Start the renderer
            ...
            Globe.prototype.addLink = function (lat1, lng1, lat2, lng2, event, $scope) {
                ...
                $scope.events.forEach(function (activeEvent) {
                    ...
                    ((activeGeoVerticies[0].equals(curveGeoVerticies[0])) &&
                        (activeGeoVerticies[activeGeoVerticies.length - 1].equals(curveGeoVerticies[curveGeoVerticies.length - 1]))) || ((activeGeoVerticies[0].equals(curveGeoVerticies[curveGeoVerticies.length - 1])) &&
            ...
            Globe.prototype.createSphere = function (x, y, z, event) {
                var color;
                if (event.color) {
                    color = event.color;
                }
                ...
                var sphere = new THREE.Mesh(new THREE.SphereGeometry(1, 10, 10), new THREE.MeshBasicMaterial({
                    color: color,
                    visible: false,
                    transparent: true
                }));
            ...
            Globe.prototype.addEvent = function (data, event, $scope) {
            ...
            Globe.prototype.fadeIn = function (group, seconds) {
                for (var i = 0; i < group.children.length; i++) {
                    // Weee closures
                    (function (child) {
    ...
    function renderGlobe(doneCallback) {
        ...
        var manager = new THREE.LoadingManager(function onComplete() {
            setTimeout(function () {
